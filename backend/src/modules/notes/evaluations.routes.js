...
router.post(
  ...
  [
    ...
    body('numero').isInt({ min: 1 }).withMessage('Numéro invalide'),
    body('coefficient').isInt({ min: 1 }).withMessage('Coefficient invalide'),
    body('date').isISO8601().withMessage('Date invalide'),
    body('classe_id').isInt({ min: 1 }).withMessage('Classe requise'),
    body('matiere_id').isInt({ min: 1 }).withMessage('Matière requise'),
